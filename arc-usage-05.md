...
abbrev: ARC-USAGE
docname: draft-ietf-dmarc-arc-usage-05
date: 2018-04-23
...
informative:
  ...
  ARC:
    ...
    target: https://tools.ietf.org/html/draft-ietf-dmarc-arc-protocol-16
    date: 2018-07-17
    author:
      -
        ins: K. Andersen
...
document will provide guidance in these areas.
...
and/or {{-DMARC}} authentication mechanisms. In such cases ARC may provide
...
a few key message header fields - including the other ARC header fields.
...
header fields inserted into the message by previous message
handlers. Thus the information from any previous ARC participants,
including the ARC-Authentication-Results header field from the mailing
list operator, would be signed at each ADMD that handled the message.
...
An intact ARC chain is one where the ARC header fields that are present can
...
participant, and recorded as such in the ARC-Seal header field they insert.
...
not to take ARC information into account at all.
...
example the remote server delivering the message to the local ADMD is
not reflected in any ARC header fields, perhaps because they have not
implemented ARC, but they modified the message such that ARC and DKIM
signatures already in the message were invalidated.
...
1. Validate that the ARC chain, if one is already present in the message, is intact and well-formed.
2. Validate that the most recent sender matches the last entry in the ARC chain (if present).
3. Validate that the most recent sender’s DKIM signature is attached, and matches the reference to it in the ARC chain (if present).
4. Generate a new ARC Signature and add it to the message according to the ARC specification.
5. Generate a new ARC Seal and add it to the message according to the ARC specification.
...
the message even though a DMARC check did not pass.
...
The authors thanks the entire OAR-Dev group for the ongoing help, innumerable diagrams and 
